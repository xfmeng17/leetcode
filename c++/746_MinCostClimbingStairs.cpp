...
public:
	...
	* PayLast1 means pay the (n-2)-th stair, go 2 step and reach top -- solution1.
	* We pick the lesser between solution0 and solution1.
	*/
	...
	*/
    ...
    int helper6(vector<int>& cost, int n, unordered_map<int, int>& memo) {
		...
		if (n <= 1) {
			...
			memo[n] = min(helper6(cost, n-1, memo), helper6(cost, n-2, memo)) + cost[n];
		}
		return memo[n];
	}
};
