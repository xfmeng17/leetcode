...
public:
  ...
  int func1(vector<int> &price, vector<vector<int>> &special,
            vector<int> &needs) {
    return helper1(price, special, 0, needs);
  }
  int helper1(vector<int> &price, vector<vector<int>> &special,
              int specialIndex, vector<int> &needs) {
    ...
    while (canUseSpecial) {
      ...
      if (canUseSpecial) {
        useSpecialCost =
            min(useSpecialCost,
                currSepcial[currSepcial.size() - 1] * cnt +
