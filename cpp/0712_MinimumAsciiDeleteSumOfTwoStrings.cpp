  ...
  int func1(string s1, string s2) {
    ...
    for (int col = n; col >= 1; col--) {
            ...
            while (c[i][j] > 0) {
                    ...
                    cout << "col =" << col << " maxRemainSum = " << maxRemainSum
    << endl;
    }
    return s1AsciiSum + s2AsciiSum - maxRemainSum;
    */
  }
  int maxPathAsciiSum(vector<vector<int>> &c, vector<vector<int>> &b, int i,
    ...
    if (b[i][j] == UP_LEFT) {
      upLeft =
          s1[i - 1] + s2[j - 1] + maxPathAsciiSum(c, b, i - 1, j - 1, s1, s2);
    }
