    ...
    for (int n = 1; n <= maxOrder; n++) {
      ...
      for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
          ...
          if (dp1[i][j] == 0)
            continue;
          if (i - space < 0 || arr[i - space][j] == 0)
            dp1[i][j] = 0;
          if (j - space < 0 || arr[i][j - space] == 0)
            dp1[i][j] = 0;
          if (i + space >= N || arr[i + space][j] == 0)
            dp1[i][j] = 0;
          if (j + space >= N || arr[i][j + space] == 0)
            dp1[i][j] = 0;
  ...
  int func3(int N, vector<vector<int>> &mines) {
    ...
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < N; j++) {
        if (grid[i][j] == 1) {
          int len = 1;
          while (i - len >= 0 && grid[i - len][j] == 1 && i + len < N &&
                 grid[i + len][j] == 1 && j - len >= 0 &&
                 grid[i][j - len] == 1 && j + len < N &&
                 grid[i][j + len] == 1) {
            len += 1;
          }
          res = max(res, len);
