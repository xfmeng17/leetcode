    	...
    	for (int k = 1; k <= K; k++) {
    		for (int i = 0; i < N; i++) {
    			for (int j = 0; j < N; j++) {
    				dp1[i][j] = 0.0;
    				// ** check 8 direction
    				if (i-2 >= 0 && j-1 >= 0) dp1[i][j] += dp2[i-2][j-1] / 8.0;
    				if (i-1 >= 0 && j-2 >= 0) dp1[i][j] += dp2[i-1][j-2] / 8.0;
    				if (i+1 < N && j-2 >= 0) dp1[i][j] += dp2[i+1][j-2] / 8.0;
    				if (i+2 < N && j-1 >= 0) dp1[i][j] += dp2[i+2][j-1] / 8.0;
    				if (i+2 < N && j+1 < N) dp1[i][j] += dp2[i+2][j+1] / 8.0;
    				if (i+1 < N && j+2 < N) dp1[i][j] += dp2[i+1][j+2] / 8.0;
    				if (i-1 >= 0 && j+2 < N) dp1[i][j] += dp2[i-1][j+2] / 8.0;
    				if (i-2 >= 0 && j+1 < N) dp1[i][j] += dp2[i-2][j+1] / 8.0;
    			}
    		}
    		// ** copy dp1 to dp2
    		for (int i = 0; i < N; i++) {
