...
public:
   ...
   * According to "Algorithms, 4th Edition => 5.3 Substring Search => Rabin-Karp
   * fingerprint search" Onile implement:
   * https://algs4.cs.princeton.edu/53substring/  => RabinKarp.java
   *
   * My notes:
   * #1 Use `const char*` instead `string&` for text to pass into the functions.
   * #2 Should use string type `unordered_set<string> set` not `long type` to
   * avoid collision. #3 Base on #2, should do `RK_Recheck` when hash value is
   * equal for sure. #4 Do #2 and #3 will make the code too slow to pass. #5
   * There are a lot of thing can be done to speed up my implement, for
   * readable, I just leave it this way.
   */
  ...
  long RK_Shift(const char *text, int i, int M, int R, long Q, long RM,
                long last_hash) {
    last_hash = (last_hash + Q - RM * text[i] % Q) % Q;
    last_hash = (last_hash * R + text[i + M]) % Q;
    return last_hash;
  }
  // Honest, we should always do Recheck, but will make "a.....a" case
  // Time-Limit-Exceeded
